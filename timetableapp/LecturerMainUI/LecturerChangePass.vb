...
Public Class LecturerChangePass
    Private Sub btnReset_Click(sender As Object, e As EventArgs) Handles btnReset.Click
        Try
            ...
            Dim da1 As New OleDb.OleDbDataAdapter("SELECT LecturerEmail, LecturerPassword FROM LecturerAccount WHERE LecturerEmail ='" & Trim(txtUserMail.Text) & "' AND LecturerPassword ='" & Trim(txtPassword.Text) & "'", con)
            ' CHECK IF PASSWORD LENGTH IS MORE THAN 5 CHARACTER
            If txtNewPass.Text.Length < 5 Then
                MsgBox("The New Password MUST be of Atleast 5 Characters", MsgBoxStyle.Exclamation)
                txtNewPass.Text = ""
                txtConPass.Text = ""
                ' CHECK IF THE OLD PASSWORD = NEW PASSWORD
            ElseIf txtPassword.Text = txtNewPass.Text Then
                MsgBox("The New Password is Same As Old Password", MsgBoxStyle.Exclamation)
                txtNewPass.Text = ""
                txtNewPass.Focus()
            Else
                ...
                If txtUserMail.Text.Trim.Length > 0 AndAlso txtPassword.Text.Trim.Length > 0 AndAlso txtNewPass.Text.Trim.Length > 0 AndAlso txtConPass.Text.Trim.Length > 0 Then
                    If (txtNewPass.Text = txtConPass.Text) Then
                        If da1.Fill(ds1) Then
                            ...
                            con.Open()
                            cb = New OleDb.OleDbCommand("UPDATE LecturerAccount SET LecturerPassword ='" & txtNewPass.Text & "' WHERE LecturerEmail ='" & txtUserMail.Text & "' ", con)
                            ra = cb.ExecuteNonQuery()
                            MessageBox.Show("Password Changed Successfully, you may proceed Login now.")
                            con.Close()
                            Me.Hide()
                        Else
        ...
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Critical, "ChangePassword | Oledb Error")
        End Try
    End Sub
