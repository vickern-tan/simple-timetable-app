...
Public Class LecturerChangeUsername
    ...
    Private Sub LecturerChangeUsername_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub btncancel2_Click(sender As Object, e As EventArgs) Handles btncancel2.Click
        Try
            ...
            txtStudentMail.Text = dr("LecturerEmail")
            con.Close()
    ...
    Sub RefreshUsername()
        Try
            ...
            ' DISPLAY USERNAME, FULLNAME IN LECTURER PROFILE PAGE
            txtUsername.Text = dr("LecturerUsername")
            con.Close()
    ...
    Sub UpdateUsername()
        Try
            ...
            Dim da1 As New OleDb.OleDbDataAdapter("SELECT * FROM LecturerAccount WHERE LecturerUsername ='" & Trim(txtUsername.Text) & "'", con)
            ' CHECK IF USERNAME LENGTH IS MORE THAN 5 CHARACTER
            ' CHECK IF THERE IS ANY EMPTY SPACE
            If txtUsername.Text.Length < 5 Then
                MsgBox("Username must be atleast 5 characters!", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly, "Username | Update-fail")
                txtUsername.Text = ""
                txtUsername.Focus()
            Else
                da1.Fill(ds1)
                ...
                con.Open()
                cb = New OleDb.OleDbCommand("UPDATE LecturerAccount SET LecturerUsername ='" & txtUsername.Text & "' WHERE LecturerID ='" & strID & "' ", con)
                ra = cb.ExecuteNonQuery()
                MsgBox("Username has been updated.", MsgBoxStyle.Information Or MsgBoxStyle.OkOnly, "Username | Update Successful")
                con.Close()
                RefreshUsername()
                txtUsername.Enabled = False
                btnEdit1.Visible = True
    ...
    Sub RefreshEmail()
        Try
            ...
            con.Open()
            newsql = "SELECT LecturerEmail FROM LecturerAccount WHERE LecturerID = '" & strID & "'"
            Dim cmd As New OleDb.OleDbCommand(newsql, con)
            Dim dr As OleDb.OleDbDataReader = cmd.ExecuteReader
            dr.Read()
    ...
    Sub UpdateUsermail()
        Try
            Dim ds1 As New DataSet
            Dim da1 As New OleDb.OleDbDataAdapter("SELECT * FROM LecturerAccount WHERE LecturerEmail ='" & Trim(txtStudentMail.Text) & "'", con)
            ...
            If System.Text.RegularExpressions.Regex.IsMatch(txtStudentMail.Text, par) Then
                ...
                ' UPDATE DATABASE RECORDS
                con.Open()
                cb = New OleDb.OleDbCommand("UPDATE LecturerAccount SET LecturerEmail ='" & txtStudentMail.Text & "' WHERE LecturerID ='" & strID & "' ", con)
                ra = cb.ExecuteNonQuery()
                MsgBox("Email has been updated.", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Email | Update Successful")
                con.Close()
                RefreshEmail()
                txtStudentMail.Enabled = False
                btnEdit2.Visible = True
